...
int I2c_getKey()
{
	...
	for (int i = 0; i < 4; ++i) {                                   //Go through rows 0-3
		if (not ioport.digitalRead(i)) {                        //If a row is low go check the column
			int c = C1;                                     //Start at Column 1
			while (not ioport.digitalRead(i))               //if input is still low, happens when column is HIGH. First read will always be LOW
				ioport.digitalWrite(c++, HIGH);         //go to next column and set it HIGH
			int col = c;
			for (c = 4; c < col; ++c)                       //go though all columns
...
/*
 * I2c_getCharachter returns the charachter of the key pressed as defined by the keys array below
 */
...
/*
 * 500Pulse is true for 500ms every second, 500Impulse is high for 1 cycle every 500ms
 */
